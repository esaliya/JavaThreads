...
$BUILD/bin/mpirun --report-bindings --hostfile nodes.txt -np $(($ppn*$nodes)) --map-by ppr:$ppn:node:SPAN --bind-to $bindto --rank-by core java -cp $cp org.saliya.javathreads.computeonly.Program $tpp $iterations

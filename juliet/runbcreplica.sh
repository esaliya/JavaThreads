...
mpirun --report-bindings --hostfile nodes.txt -np $ppn --map-by ppr:$ppn:node:SPAN --bind-to $bindto java -cp $cp org.saliya.javathreads.BCReplica $threadCount $coreCount $isBind $useHJ $iterations $lrt

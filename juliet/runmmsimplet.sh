...
$BUILD/bin/mpirun --hostfile nodes.txt -np $(($ppn*$nodes)) --map-by ppr:$ppn:node:SPAN --bind-to $bindto --rank-by core java -cp $cp org.saliya.javathreads.array.ProgramSimpleThreads $tpp $iterations $rows $cols $dim $hj

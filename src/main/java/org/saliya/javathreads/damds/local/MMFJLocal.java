...
public class MMFJLocal{
    ...
    public static void main(String[] args)
        ...
        MMUtils.printMessage("Running in Local Data FJ Mode");
...
//        IntStream.range(0, threadCount*ParallelOps.worldProcsCount).forEach(i -> MMUtils.printMessage("Rank " + (i/threadCount) + " Thread " + (i%threadCount) + " comp time " + times.get(i) + " ms" ));
    ...
    public static void mmLoopLocalData(int[] threadRowCounts) throws MPIException {
        IntStream.range(0, ParallelOps.threadCount).forEach(i -> mmWorkers[i] =
                new MMWorker(i, globalColCount, targetDimension, blockSize, threadRowCounts[i]));
